import React from 'react'

const Faq = () => {
    return (
        <div className='md:px-20 px-5 bg-black'>
            <div className='w-full text-5xl mb-5'>
                <div className='mb-6'>

                    <h1 >FAQ</h1>
                </div>
                <div className='w-full  border-white border-2'></div>
            </div>
            <div className='md:text-3xl text-xl '>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" defaultChecked />
                    <div className="collapse-title">How do I create an account?</div>
                    <div className="collapse-content ">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Reiciendis corrupti exercitationem voluptate vero sed, recusandae incidunt nemo necessitatibus placeat, magni ipsa numquam illum saepe. Deserunt inventore corporis eius laboriosam, sequi eos fugit sed neque cumque iusto optio, dolorem ratione necessitatibus.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" />
                    <div className="collapse-title ">I forgot my password. What should I do?</div>
                    <div className="collapse-content">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sapiente consequatur dignissimos beatae. Esse natus, nam sint repudiandae dignissimos eaque voluptatibus itaque magni perferendis ipsum! Delectus maxime quasi recusandae itaque praesentium nobis est ducimus? Quis odit asperiores veniam cumque incidunt rem.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" />
                    <div className="collapse-title">How do I update my profile information?</div>
                    <div className="collapse-content">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Distinctio cumque dolore minima est asperiores, autem natus sed sit rerum accusantium vel maxime ea, enim omnis in dolor labore nisi quisquam? Laboriosam ab magni libero dicta. Odio ipsa quod vel soluta.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" defaultChecked />
                    <div className="collapse-title">How do I create an account?</div>
                    <div className="collapse-content ">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Reiciendis corrupti exercitationem voluptate vero sed, recusandae incidunt nemo necessitatibus placeat, magni ipsa numquam illum saepe. Deserunt inventore corporis eius laboriosam, sequi eos fugit sed neque cumque iusto optio, dolorem ratione necessitatibus.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" />
                    <div className="collapse-title ">I forgot my password. What should I do?</div>
                    <div className="collapse-content">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sapiente consequatur dignissimos beatae. Esse natus, nam sint repudiandae dignissimos eaque voluptatibus itaque magni perferendis ipsum! Delectus maxime quasi recusandae itaque praesentium nobis est ducimus? Quis odit asperiores veniam cumque incidunt rem.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" />
                    <div className="collapse-title">How do I update my profile information?</div>
                    <div className="collapse-content">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Distinctio cumque dolore minima est asperiores, autem natus sed sit rerum accusantium vel maxime ea, enim omnis in dolor labore nisi quisquam? Laboriosam ab magni libero dicta. Odio ipsa quod vel soluta.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" defaultChecked />
                    <div className="collapse-title">How do I create an account?</div>
                    <div className="collapse-content ">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Reiciendis corrupti exercitationem voluptate vero sed, recusandae incidunt nemo necessitatibus placeat, magni ipsa numquam illum saepe. Deserunt inventore corporis eius laboriosam, sequi eos fugit sed neque cumque iusto optio, dolorem ratione necessitatibus.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" />
                    <div className="collapse-title ">I forgot my password. What should I do?</div>
                    <div className="collapse-content">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sapiente consequatur dignissimos beatae. Esse natus, nam sint repudiandae dignissimos eaque voluptatibus itaque magni perferendis ipsum! Delectus maxime quasi recusandae itaque praesentium nobis est ducimus? Quis odit asperiores veniam cumque incidunt rem.</div>
                </div>
                <div className="collapse collapse-arrow bg-transparent border text-white rounded-none border-white">
                    <input type="radio" name="my-accordion-2" />
                    <div className="collapse-title">How do I update my profile information?</div>
                    <div className="collapse-content">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Distinctio cumque dolore minima est asperiores, autem natus sed sit rerum accusantium vel maxime ea, enim omnis in dolor labore nisi quisquam? Laboriosam ab magni libero dicta. Odio ipsa quod vel soluta.</div>
                </div>
            </div>
        </div>
    )
}

export default Faq